...
test('requires a verified token and provisions a first-time caller', async () => {
  ...
  assert.equal(
    (await request('/api/lnbits/users', { token: 'nope' })).status,
    401,
  );
...
test('write routes refuse body fields they do not read', async () => {
  const forged = [
    [
      ...
      { amount: 25, memo: 'hi', aadObjectId: 'forged-oid' },
    ],
    [
      '/api/lnbits/wallets/wallet-1/invoices',
      { amount: 25, memo: 'hi', walletId: 'wallet-9' },
    ],
    [
      '/api/lnbits/wallets/wallet-1/payments',
      { paymentRequest: 'lnbc1', aadObjectId: 'forged-oid' },
    ],
    [
      '/api/lnbits/zaps',
      {
