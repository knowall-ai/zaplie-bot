...
const createSendZap = ({
  ...
  return async ({
    ...
    const maxAmount = Number.isSafeInteger(configuredMax) && configuredMax > 0
      ? configuredMax
      : 1_000_000;
    if (
    ...
    const operation = (async () => {
      ...
      try {
        ...
        try {
          ...
          console.error('Zap idempotency completion could not be persisted');
        }
        return result;
      } catch (error) {
        try {
          ...
          console.error('Zap idempotency failure could not be persisted');
        }
        throw error;
      }
    })();
