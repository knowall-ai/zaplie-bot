...
const pickTargetWallet = (wallets: Wallet[] | null): Wallet | null =>
  wallets?.find(wallet => wallet.name === 'Private') ?? null;
...
const SendZapsPopup: React.FC<SendZapsPopupProps> = ({
  ...
  useEffect(() => {
    ...
    loadUsers();
    // cache and setCache are stable context values.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [accounts, initialUserId]);
      ...
      if (result && result.payment_hash) {
        ...
        if (currentUserId) {
          ...
          try {
            ...
            const refreshedAllowance = refreshedWallets.find(
              wallet => wallet.name === 'Allowance',
            );
            setCurrentUserWallets({
                ...
                <p className={styles.balanceText}>
                  ...
                  <b className={styles.balanceValue}>
                    {currentUserWallets.balance === null
                      ? 'Loading...'
                      : `${currentUserWallets.balance.toLocaleString()} ${rewardsName}`}
                  </b>
                </p>
              </div>
            </div>
                  ...
                  {amountIsValid ? (
                    <>
                      ...
                      <b className={styles.summaryAmount}>
                        {numericAmount.toLocaleString()} {rewardsName}
                      </b>
