...
const pairId = (transaction: Transaction) =>
...
const WalletTransactionLog: React.FC<WalletTransactionLogProps> = ({
  ...
  useEffect(() => {
    ...
    const loadTransactions = async () => {
      ...
      if (!accountId) {
        setLoading(false);
        setError('Sign in to load your transaction history.');
        return;
      }
      ...
      try {
        ...
        const wallet = wallets.find(
          candidate => candidate.name === activeWallet,
        );
        if (!wallet) {
  ...
  }, [accountId, activeWallet, retryToken]);
