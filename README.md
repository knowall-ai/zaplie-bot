...
- [LNbits](https://www.lnbits.com) version 1.0 or higher (the shared test instance runs 1.5.6). The bot provisions users and wallets through the core Users API, which replaced the old User Manager extension, so no extension needs to be enabled. NB Credentials are no longer committed: copy `env/.env.dev.example` to `env/.env.dev` and fill in your own LNbits details (ask a maintainer for the shared development instance), or create your own instance, as specified in `Wiki` > `Setup` > `LNbits`.
