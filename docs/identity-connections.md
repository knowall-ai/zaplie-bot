...
`TAB_BACKEND_TOKEN` protects only the server-to-server routes introduced for
identity resolution and pending rewards. Existing browser-facing legacy routes
such as `/api/reward-name` still use their pre-existing placeholder middleware.
They must migrate to verified MSAL identity, with `Zaplie.Admin` authorization
for administrative writes, before this stack is production-ready. The internal
token is deliberately not accepted as a browser credential and there is no
